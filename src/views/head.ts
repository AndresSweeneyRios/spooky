...
export class HeadView extends View {
  ...
  public Draw(simulation: Simulation, lerpFactor: number): void {
    ...
    this.head.scene.position.set(
      this.position.x,
      this.position.y,
      this.position.z
    );
    this.head.scene.position.y +=
      ((Math.sin(simulation.ViewSync.TimeMS / 150) + 1) / 2) * 0.03;
    ...
    const currentHeadPosition = new THREE.Vector3().copy(
      this.head.scene.position
    );
    ...
    for (let i = 0; i < this.particles.count; i++) {
      ...
      instanceAlpha[i] = Math.max(
        0,
        (1.0 - -this.particlePositions[i].y / 2) * 0.9
      );
    ...
    const alphaAttr = this.particles.geometry.getAttribute(
      "instanceAlpha"
