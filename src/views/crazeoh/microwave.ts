...
export class MicrowaveView extends View {
  ...
  constructor(microwaveObject: THREE.Object3D, private simulation: Simulation) {
    super()
  ...
  public Draw(simulation: Simulation, lerpFactor: number): void {
    const targetAngle = this.isOpen ? MICROWAVE_OPEN_ANGLE : MICROWAVE_CLOSED_ANGLE

    // use last interaction timestamp to determine if the microwave should animate
    if (simulation.ViewSync.TimeMS - this.lastInteractionTimestamp > MICROWAVE_OPEN_SPEED) {
    ...
    const percentage = T((simulation.ViewSync.TimeMS - this.lastInteractionTimestamp) / MICROWAVE_OPEN_SPEED)

    if (!this.isOpen) {
      this.doorBone.rotation.y = MICROWAVE_OPEN_ANGLE + (MICROWAVE_CLOSED_ANGLE - MICROWAVE_OPEN_ANGLE) * percentage
    } else {
      this.doorBone.rotation.y = MICROWAVE_CLOSED_ANGLE + (targetAngle - MICROWAVE_CLOSED_ANGLE) * percentage
    }
  }
