...
export class SpawnEnforcer extends SimulationCommand {
  public Execute(simulation: Simulation): void {
    if (this.Position === null || this.Rotation === null) {
      throw new Error(
        "Position and Rotation must be set before executing SpawnEnforcer"
      );
    }
    ...
    import("../../../views/head").then(({ HeadView: _ }) => {
      ...
      const enforcerView = new HeadView(
        simulation.ThreeScene,
        simulation.Camera
      );

      enforcerView.position.set(position[0], position[1] + 0.5, position[2]);
