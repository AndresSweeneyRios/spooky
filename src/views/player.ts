...
export class PlayerView extends EntityView {
  ...
  protected updateCamera(dx: number, dy: number): void {
    const mouseSensitivityPercentageFloat =
      1.0 - parseFloat(localStorage.sensitivity);

    const mouseSensitivity =
      MIN_SENSITIVITY +
      (MAX_SENSITIVITY - MIN_SENSITIVITY) *
        (mouseSensitivityPercentageFloat / 100);
  ...
  protected getAvailableInteractionsWithinAngle(
    maxAngle: number
  ): { command: SensorCommand; entId: EntId; symbol: symbol; angle: number }[] {
    const commands =
      this.simulation.SimulationState.SensorCommandRepository.GetAvailableInteractions(
        this.EntId
      );
    const interactions = commands
      .map(({ command, entId, symbol }) => {
        const position =
          this.simulation.SimulationState.PhysicsRepository.GetPosition(entId);
        const playerPosition =
          this.simulation.SimulationState.PhysicsRepository.GetPosition(
            this.EntId
          );
  ...
  protected handleJustPressed(payload: JustPressedEvent): void {
    ...
    let closestInteraction = availableInteractions.reduce(
      (closest, interaction) => {
    ...
    this.simulation.SimulationState.Commands.push(
      closestInteraction.command.Command
    );
    payload.consume();
    if (closestInteraction.command.Once) {
      this.simulation.SimulationState.SensorCommandRepository.DeleteSensorCommand(
        closestInteraction.entId,
        closestInteraction.symbol
      );
    }
  }
  ...
  public disableControls(): void {
    ...
    this.cleanupEvents();
    this.cleanupEvents = () => {};
    this.simulation.SimulationState.MovementRepository.SetDirection(
      this.EntId,
      [0, 0, 0]
    );
  ...
  public Draw(simulation: Simulation, lerpFactor: number): void {
    ...
    const basePosition = new THREE.Vector3(
      ...
      lerpedPosition[1],
      lerpedPosition[2]
    ).add(new THREE.Vector3(...this.cameraPositionOffset));
    const offset = new THREE.Vector3(...this.cameraOffset).applyQuaternion(
      this.simulation.Camera.quaternion
    );
    basePosition.add(offset);
    this.simulation.Camera.position.set(
      basePosition.x,
      basePosition.y,
      basePosition.z
    );
  ...
  public Update(simulation: Simulation): void {
    document
      .querySelector(".caseoh-interactable")
      ?.setAttribute("is-hidden", "true");
    ...
    if (localDirection.length() > 0) {
      ...
      if (
        !this.lastFootstepTime ||
        now - this.lastFootstepTime >= footstepInterval
      ) {
    ...
    state.MovementRepository.SetDirection(this.EntId, [
      localDirection.x,
      localDirection.y,
      localDirection.z,
    ]);
    ...
    const isInteractable =
      this.getAvailableInteractionsWithinAngle(20).length > 0;
    document
      .querySelector(".caseoh-interactable")
      ?.setAttribute("is-hidden", isInteractable ? "false" : "true");
  }
