...
export class FridgeView extends View {
  ...
  constructor(fridgeObject: THREE.Object3D, private simulation: Simulation) {
    super()
  ...
  public Draw(simulation: Simulation, lerpFactor: number): void {
    ...
    if (simulation.ViewSync.TimeMS - this.lastInteractionTimestamp > FRIDGE_OPEN_SPEED) {
    ...
    const percentage = T((simulation.ViewSync.TimeMS - this.lastInteractionTimestamp) / FRIDGE_OPEN_SPEED)

    if (!this.isOpen) {
      this.doorBone.rotation.y = FRIDGE_OPEN_ANGLE + (FRIDGE_CLOSED_ANGLE - FRIDGE_OPEN_ANGLE) * percentage
    } else {
      this.doorBone.rotation.y = FRIDGE_CLOSED_ANGLE + (targetAngle - FRIDGE_CLOSED_ANGLE) * percentage
    }
  }
