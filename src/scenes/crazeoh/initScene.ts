...
import {
  simulationPlayerViews,
  InteractionsChangedPayload,
} from "../../views/player";
...
export const initScene = async (map: Promise<THREE.Object3D>) => {
  ...
  simulation.ViewSync.AddAuxiliaryView(
    new (class extends View {
      public Draw(): void {
        ...
        if (!state.gameStarted || (state.inDialogue && !state.outro))
          camera.rotateY(-0.1 * simulation.ViewSync.DeltaTime);
  ...
  const createFlashlight = () => {
    ...
    simulation.ViewSync.AddAuxiliaryView(
      new (class FlashlightPositionManager extends View {
        public Draw(): void {
  ...
  simulation.ViewSync.AddAuxiliaryView(
    new (class PlayerControlsManager extends View {
      public Draw(): void {
        if (
          state.gameStarted &&
          !state.picking &&
  ...
  const interactionsChangedHandler = (
    interactions: InteractionsChangedPayload
  ) => {
    ...
    const selectedObjects = closestInteraction?.command.Owner
      ? [closestInteraction.command.Owner]
      : [];

    currentOutlinePass?.selectedObjects.splice(
      0,
      currentOutlinePass.selectedObjects.length
    );
  ...
  until(() =>
    Boolean(simulationPlayerViews[simulationIndex]?.interactionEmitter)
  ).then(() => {
    simulationPlayerViews[simulationIndex]!.interactionEmitter.on(
      "interactionsChanged",
  ...
  const cleanup = () => {
    simulationPlayerViews[simulation.SimulationIndex]?.interactionEmitter.off(
      "interactionsChanged",
      interactionsChangedHandler
    );
