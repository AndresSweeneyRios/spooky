...
const polaroidEl = document.querySelector(
  ".caseoh-polaroid-overlay.ingame .background"
) as HTMLImageElement;
const polaroid2El = document.querySelector(
  "#caseoh-decision .caseoh-polaroid-overlay .background"
) as HTMLImageElement;
const cameraHintEl = document.querySelector(
  ".caseoh-camera-hint"
) as HTMLElement;
...
const setupGarageScream = (simulation: Simulation) => {
  garageScreamAudioPromise.then((audio) => {
    ...
    const mesh = simulation.ThreeScene.getObjectByName(
    ...
    simulation.ViewSync.AddAuxiliaryView(
      new (class extends View {
        ...
        public Draw(): void {
          ...
          audio.setVolume(
            !state.gameStarted || state.picking || state.inDialogue ? 0 : 0.05
          );
        }
...
const setupEat = (simulation: Simulation, scene: THREE.Scene) => {
  const foods = [
    ...
    "strawberyshake",
    "cereal",
    "23b099929e614d9a927b4ec8f3d72063fbx",
    "donuts",
  ];
...
const setupBurgerKing = (simulation: Simulation, scene: THREE.Scene) => {
  const mesh = scene
    .getObjectByName("Sketchfab_model001")!
    .getObjectByProperty("type", "Mesh") as THREE.Mesh;
...
export const init = async () => {
  ...
  const playerView = await player.createPlayer(
    simulation,
    [2, 0, -6],
    [0, 0, 0]
  );
  ...
  const teleportPlayer = () => {
    ...
    simulation.SimulationState.PhysicsRepository.SetPosition(
      playerView!.EntId,
      [playerSpawnPosition.x, 0.5, playerSpawnPosition.z]
    );
    ...
    const yaw = Math.atan2(
      playerSpawnPosition.x - tempVec3.x,
      playerSpawnPosition.z - tempVec3.z
    );
    const pitch = Math.atan2(
      tempVec3.y - playerSpawnPosition.y,
      Math.hypot(
        playerSpawnPosition.x - tempVec3.x,
        playerSpawnPosition.z - tempVec3.z
      )
  ...
  const justPressed = (payload: JustPressedEvent) => {
    if (payload.action !== "mainAction1" || state.picking || state.inSettings)
      return;
    ...
    if (state.anomaly) {
      const pPos = simulation.SimulationState.PhysicsRepository.GetPosition(
        playerView!.EntId
      );
      if (pPos) {
        tempVec3.set(
